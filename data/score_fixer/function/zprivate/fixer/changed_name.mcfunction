# Case 1: The name is still present in the map
...
# Copy scores
    ...
    execute if score #ScoreFixer.ObjectiveCount ScoreFixer matches 1.. run function score_fixer:zprivate/fixer/copy_scores with storage score_fixer:zprivate Temp.Objectives[-1]
